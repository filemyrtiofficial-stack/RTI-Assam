...
export const PDFDownloadModal: React.FC<PDFDownloadModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // For Assam, PDFs are in RTI-Templates folder directly (format: "RTI-Templates/filename.pdf")
      const pathParts = pdfPath.split('/');
      const isAssamFormat = pathParts.length === 2 && pathParts[0] === 'RTI-Templates';
      ...
      if (isAssamFormat) {
        // Assam PDFs: RTI-Templates/filename.pdf
        state = 'assam';
        category = 'RTI-Templates';
        filename = encodeURIComponent(pathParts[1]);
      } else {
        // Other states: state/category/filename.pdf
        state = pathParts[0]; // e.g., "delhi" or "telangana"
      ...
      try {
        ...
        if (response && response.ok) {
          ...
          const downloadFilename = departmentName
            .replace(/RTI\s+(Delhi|Telangana|Assam|Karnataka)\s+/gi, '')
            .replace(/\s+/g, '_')
            .concat('.pdf');
      ...
      if (!downloadSuccess) {
        try {
          ...
          // For Assam, PDFs are directly in RTI-Templates folder in public
          let publicPath: string;
          if (isAssamFormat) {
            publicPath = `/assets/PDF/${decodedCategory}/${decodedFilename}`;
          } else {
          ...
          // For Assam, public folder approach might fail, continue to direct import
